...
from src.portfolio_generator import (
    _aggregate_to_symbols,
    _apply_portfolio_governor,
    _build_portfolio_records,
    _build_portfolio_dataframe,
)
...
def get_trading_dates_in_range(
    ...
    feature_base = config.pair_state_dir / f"version={config.feature_version}"
...
def get_next_trading_date(
    ...
    # Get the next date where we have price data for this symbol
    df = read_daily_bars(config, symbol)
    if df is None or df.is_empty():
        return None

    future_dates = df.filter(pl.col("date") > current_date).select("date").to_series().to_list()
    if not future_dates:
        return None
...
def run_backtest(
    ...
    # Get trading dates
    trading_dates = get_trading_dates_in_range(config, start_date, end_date)
    ...
    backtest_metrics = _compute_backtest_metrics(daily_results, trading_dates)
...
def _run_single_day(
    ...
    temp_logs = []
    symbol_notionals, _, _ = _apply_portfolio_governor(
    ...
    # Compute PnL (using next day returns)
    # We need to look up next day prices and compute returns
    pnl_gross = 0.0
    for symbol in symbol_notionals:
        ...
        next_date = get_next_trading_date(config, asof_date, symbol)

        if price_today is None or next_date is None:
            continue

        price_next = get_price_on_date(config, symbol, next_date)
    ...
    logs.append(
        ...
        f"pnl_gross={pnl_gross:,.0f}, pnl_net={pnl_net:,.0f}"
    )
